# 📱 Guia Completo - Assistente Julia WhatsApp

## ℹ️ Entenda como funciona Pessoal vs PJ

**Padrão (Pessoal):** Quando você não especificar, todas as transações vão para a conta **Pessoal**.

**PJ/Empresa:** Use palavras como **"empresa"**, **"PJ"**, **"negócio"**, **"empresarial"** para lançar na conta PJ.

---

## ⚡ O que você pode fazer

### 📝 Registrar Despesas
Adicione gastos já realizados

✅ "Gastei 230 no mercado"

✅ "Paguei 85 reais de remédio"

✅ "Almocei no restaurante 45 reais, alimentação"

✅ "Abasteci o carro com 200 reais de gasolina na empresa"

✅ "Paguei 95 de Uber transporte"

✅ "Comprei material de escritório 120 na empresa"

✅ "Gastei 80 com diarista, serviços domésticos"

---

### 💰 Registrar Receitas
Adicione entradas de dinheiro

✅ "Recebi 3500 de salário"

✅ "Entrou 800 de freelance no perfil empresarial"

✅ "Recebi 260 de comissão de vendas na empresa"

✅ "Recebi 180 de reembolso do plano de saúde"

✅ "Vendi um produto por 450 reais na empresa"

✅ "Recebi aluguel 1500"

✅ "Entrou 1200 de consultoria na conta empresarial"

---

### 📋 Consultar Extrato
Veja seus gastos e receitas

✅ "Quero ver minhas despesas"

✅ "Extrato do mês da empresa"

✅ "Gastos de hoje"

✅ "Despesas da semana no perfil empresarial"

✅ "Quanto gastei em alimentação este mês?"

✅ "Minhas receitas de ontem na empresa"

✅ "Gastos com transporte dos últimos 7 dias"

---

### 💵 Ver Saldo e Relatórios
Acompanhe suas finanças

✅ "Qual meu saldo?"

✅ "Resumo financeiro da empresa"

✅ "Relatório de novembro"

✅ "Balanço do mês na conta empresarial"

✅ "Como estão minhas finanças?"

---

### 📅 Lançamentos Futuros
Gerencie contas a pagar e recorrentes

✅ "Tenho IPTU de 70 reais vence dia 10/01/2026 e são 3 parcelas"

✅ "Tenho Netflix mensal recorrente de 29,90 e data final é 10/01/2027"

✅ "Conta de luz vence dia 15 próximo mês 150 reais"

✅ "Aluguel recorrente 1200 todo dia 5"

---

### 💳 Cartão de Crédito
Controle faturas e limites

✅ "Comprei roupa 150 no cartão Nubank"

✅ "Gastei 300 parcelado em 3x no cartão Itaú"

✅ "Qual o valor da fatura atual do cartão Nubank?"

✅ "Quanto tenho de limite disponível no Itaú?"

---

## 📖 Como Funciona

### 1️⃣ Salve o Contato
Adicione o número do assistente na sua agenda do celular

### 2️⃣ Inicie a Conversa
Clique no botão acima ou envie uma mensagem diretamente

### 3️⃣ Converse Naturalmente
Fale como se estivesse conversando com um amigo

---

## 💡 Dicas Importantes

### 🔹 Registros Imediatos (Já Aconteceu)
Use **"Gastei"**, **"Comprei"**, **"Recebi"** para transações que você já fez AGORA:
- ✅ "Gastei 50 no mercado"
- ✅ "Comprei 150 na farmácia"
- ✅ "Recebi 3000 de salário"

### 🔹 Lançamentos Futuros (Vai Acontecer)
Use **"Tenho que pagar"**, **"Agendar"**, **"Programar"** para pagamentos futuros:
- ✅ "Tenho que pagar 1500 de aluguel dia 10"
- ✅ "Agendar conta de luz de 200 para dia 15"
- ✅ "Preciso pagar 500 de dentista semana que vem"

### 🔹 Confirmar Pagamento Agendado
Use **"Paguei"**, **"Quitei"** para confirmar algo que estava agendado:
- ✅ "Paguei o aluguel hoje"
- ✅ "Quitei a parcela 2 do IPTU"
- ✅ "Efetuei o pagamento da internet"

### 🔹 Cartão de Débito vs Crédito

**Débito:** Registra AGORA e desconta do saldo
- ✅ "Gastei 80 no supermercado com cartão de débito Nubank"
- ✅ "Comprei 45 na farmácia débito Itaú"

**Crédito:** Fica em aberto até pagar a fatura
- ✅ "Comprei 250 de roupas no crédito Nubank"
- ✅ "Gastei 180 no restaurante com cartão de crédito Itaú"

### 🔹 Parcelamento em Cartão de Crédito
- ✅ "Comprei notebook de 3000 em 12 parcelas no crédito Nubank"
- ✅ "Parcelei 600 em 6x no cartão de crédito Itaú"
- ✅ "Compra de 1200 dividida em 10 parcelas crédito Inter"

### 🔹 transacoes na Contas Bancária Específica
Mencione a conta para vincular:
- ✅ "Recebi 5000 de salário na conta Nubank"
- ✅ "Gastei 800 de aluguel pela conta Itaú"
- ✅ "Transferi 500 da conta Inter"
- ✅ "Agendar 1500 na conta Bradesco PJ para dia 10"

### 🔹 Lançamentos Recorrentes

**Com data final:**
- ✅ "Aluguel de 1500 todo dia 10 até 31/12/2025"
- ✅ "Salário de 5000 recorrente até dezembro de 2025"
- ✅ "Assinatura de 50 mensal até 30/06/2025"
- ✅ "Curso de 200 todo mês até março"
- ✅ "Internet empresa 800 mensal de janeiro a junho"

### 🔹 Consultas e Relatórios

**Saldo de Conta Bancária:**
- ✅ "Qual o saldo da conta Nubank?"
- ✅ "Saldo do Itaú"
- ✅ "Quanto tenho na conta Inter?"
- ✅ "Saldo de todas as contas"

**Fatura de Cartão de Crédito:**
- ✅ "Qual a fatura do Nubank?"
- ✅ "Fatura do cartão Itaú este mês"
- ✅ "Quanto tenho para pagar no crédito?"
- ✅ "Valor da fatura de dezembro"

**Relatórios por Categoria:**
- ✅ "Quanto gastei com alimentação este mês?"
- ✅ "Gastos com transporte em novembro"
- ✅ "Mostre minhas últimas 5 transações"
- ✅ "Extrato da semana passada"

**Projeções Futuras:**
- ✅ "Previsão de fluxo de caixa para os próximos 15 dias"
- ✅ "Como será meu fluxo de caixa este mês?"
- ✅ "O que tenho para pagar esta semana?"
- ✅ "Quais parcelas faltam do notebook?"
- ✅ "Resumo das minhas assinaturas"

### 🔹 Excluir Transação
Quando você registra uma transação, o sistema retorna um ID:
- ✅ "Excluir transação 12345"

**Se não conseguir excluir pelo WhatsApp:**
- Acesse: https://www.assistentejulia.com.br/dashboard
- Encontre a transação e clique em "Excluir"

### 🔹 Diferença PF e PJ

**PF (Pessoal):** Apenas descreva normalmente
- ✅ "Gastei 100 no mercado"
- ✅ "Recebi 5000 de salário"

**PJ (Empresa):** Use palavras-chave: **PJ**, **empresa**, **negócio**
- ✅ "Gastei 500 de fornecedor PJ"
- ✅ "Recebi 10000 de cliente empresa"
- ✅ "Agendar 3000 de fornecedor empresa para dia 15"

---

## 🌐 Funcionalidades Exclusivas da Plataforma Web

Acesse: **https://www.assistentejulia.com.br/dashboard**

### 📊 Gráficos e Dashboards
- Visualize seus gastos em gráficos interativos
- Acompanhe evolução mensal
- Compare períodos

### 💰 Gestão de Investimentos
**Onde:** https://www.assistentejulia.com.br/dashboard/investimentos
- Registre posições em ações
- Acompanhe criptomoedas
- Gerencie fundos de investimento (FI)
- Visualize rentabilidade

### 🎯 Metas Financeiras
**Onde:** https://www.assistentejulia.com.br/dashboard/metas
- Meta de gastos (ex: gastar no máximo R$ 2.000 com alimentação)
- Meta de ganhos (ex: receber pelo menos R$ 10.000 este mês)
- Meta por categoria (ex: limitar transporte a R$ 500)

### 💳 Gestão Completa de Cartões
**Onde:** https://www.assistentejulia.com.br/dashboard/cartoes/
- Ver extrato completo da fatura
- Pagar/Fechar fatura do mês
- Visualizar histórico de faturas
- Gerenciar múltiplos cartões

### 🏦 Gestão de Contas Bancárias
**Onde:** https://www.assistentejulia.com.br/dashboard
- Visualizar todas as suas contas
- **Separação automática:** Contas PF e PJ organizadas separadamente
- Ver saldo de cada conta
- Acompanhar histórico de transações
- Adicionar, editar ou remover contas

### 👥 Plano Casal - Compartilhamento
**Onde:** https://www.assistentejulia.com.br/dashboard/configuracoes
1. Acesse Configurações > Compartilhamento
2. Adicione o telefone do seu cônjuge
3. Defina as permissões
4. Seu cônjuge se cadastra com o mesmo e-mail e telefone
5. Pronto! Vocês compartilham as informações financeiras

### 👔 Adicionar Funcionário (PJ)
**Onde:** https://www.assistentejulia.com.br/dashboard/configuracoes
- Mesmo processo do Plano Casal
- Adicione telefone do funcionário
- Defina autorizações específicas
- Funcionário se cadastra com os mesmos dados

### 🔔 Lembretes Automáticos
**Onde:** https://www.assistentejulia.com.br/dashboard/configuracoes
- Ative notificações de contas a pagar (CP) e receber (CR)
- Defina quando lembrar:
  - No dia do vencimento
  - 1 dia antes
  - 3 dias antes
  - 1 semana antes

### 🌍 Configurações Regionais
**Onde:** https://www.assistentejulia.com.br/dashboard/configuracoes

**Idiomas disponíveis:**
- 🇧🇷 Português (Brasil)
- 🇪🇸 Espanhol
- 🇺🇸 Inglês

**Moedas disponíveis:**
- BRL (Real Brasileiro)
- USD (Dólar Americano)
- PYG (Guarani Paraguaio)
- ARS (Peso Argentino)
- EUR (Euro)

### 📱 Instalar como Aplicativo (PWA)
**Instruções:** https://www.assistentejulia.com.br/dashboard/instalacao
1. Acesse a plataforma pelo navegador do celular
2. No menu, procure "Adicionar à tela inicial"
3. Confirme
4. Pronto! Ícone do Assistente Julia na tela inicial

---

## 📋 Categorias Disponíveis

- 🍔 Alimentação
- 🚗 Transporte
- 🏠 Moradia
- 💊 Saúde
- 🎓 Educação
- 🎬 Lazer
- 👕 Vestuário
- 💼 Trabalho
- 💰 Investimentos
- 🔧 Serviços
- 📱 Tecnologia
- 🎁 Presentes
- 🏥 Seguros
- 💳 Taxas e Impostos
- 📦 Outros

*O sistema categoriza automaticamente. Se precisar, reclassifique pela plataforma web.*

---

## ❓ Resolvendo Problemas Comuns

**"Não consegui excluir uma transação pelo WhatsApp"**
→ Acesse https://www.assistentejulia.com.br/dashboard e exclua por lá

**"A categoria está errada"**
→ Edite pela plataforma web em https://www.assistentejulia.com.br/dashboard

**"Não recebi o lembrete"**
→ Verifique se as notificações estão ativadas em https://www.assistentejulia.com.br/dashboard/configuracoes

**"Minha fatura não está correta"**
→ Acesse https://www.assistentejulia.com.br/dashboard/cartoes/ e verifique o extrato completo

**"Não consigo acessar a plataforma web"**
→ Certifique-se de usar o **mesmo e-mail e telefone** do WhatsApp no cadastro

**"Como diferenciar PF e PJ?"**
→ Use palavras-chave (PJ, empresa, negócio) para transações empresariais

**"Não sei se uso 'gastei' ou 'paguei'"**
→ **"GASTEI"** = já aconteceu agora | **"PAGUEI"** = confirmar algo agendado | **"TENHO QUE PAGAR"** = agendar futuro

**"Onde vejo minhas contas bancárias?"**
→ Acesse https://www.assistentejulia.com.br/dashboard - todas as contas estão lá, separadas por PF e PJ

---

## 🔗 Links Importantes

- 🏠 **Dashboard Principal:** https://www.assistentejulia.com.br/dashboard
- 📝 **Cadastro:** https://www.assistentejulia.com.br/cadastro
- 🏦 **Gestão de Contas:** https://www.assistentejulia.com.br/dashboard
- 💳 **Cartões:** https://www.assistentejulia.com.br/dashboard/cartoes/
- 📈 **Investimentos:** https://www.assistentejulia.com.br/dashboard/investimentos
- 🎯 **Metas:** https://www.assistentejulia.com.br/dashboard/metas
- ⚙️ **Configurações:** https://www.assistentejulia.com.br/dashboard/configuracoes
- 📱 **Instalação:** https://www.assistentejulia.com.br/dashboard/instalacao

---

## 💬 Exemplos Práticos Completos

### Cenário 1: Registro de Despesa Simples
**Você:** "Gastei 230 no mercado"
**Sistema:** Registra a transação imediatamente na categoria Alimentação

### Cenário 2: Receita Empresarial
**Você:** "Recebi 3500 de cliente na empresa"
**Sistema:** Registra entrada na conta PJ

### Cenário 3: Parcelamento em Cartão
**Você:** "Comprei notebook de 3000 em 12 parcelas no crédito Nubank"
**Sistema:** Cria 12 lançamentos futuros, um para cada mês

### Cenário 4: Conta Recorrente
**Você:** "Aluguel de 1500 todo dia 10 recorrente"
**Sistema:** Cria lançamento mensal automático sem data final

### Cenário 5: Consulta de Gastos
**Você:** "Quanto gastei com alimentação este mês?"
**Sistema:** Soma todos os gastos da categoria Alimentação do mês atual

### Cenário 6: Confirmar Pagamento Agendado
**Você:** "Paguei o aluguel hoje"
**Sistema:** Busca o lançamento agendado de aluguel e marca como pago

### Cenário 7: Lançamento com Conta Específica
**Você:** "Agendar 1500 na conta Bradesco PJ para dia 10"
**Sistema:** Cria lançamento futuro vinculado à conta Bradesco PJ

### Cenário 8: Consulta de Fatura
**Você:** "Qual a fatura do Nubank?"
**Sistema:** Mostra valor total, data de fechamento e vencimento da fatura atual

---

## 🎓 Dicas para Melhor Uso

1. **Seja natural:** Escreva como você fala. O sistema entende linguagem natural.

2. **Use palavras-chave corretas:**
   - "GASTEI" = transação imediata (já aconteceu)
   - "TENHO QUE PAGAR" = agendamento futuro
   - "PAGUEI" = baixa de pagamento agendado

3. **Use palavras-chave para PJ:** Sempre inclua "PJ", "empresa" ou "negócio" em transações empresariais.

4. **Especifique a conta/cartão:** Mencione qual conta ou cartão para melhor organização.

5. **Consulte relatórios regularmente:** Use o WhatsApp para consultas rápidas e a web para análises detalhadas.

6. **Configure lembretes:** Nunca mais esqueça de pagar uma conta!

7. **Use o plano casal:** Compartilhe com seu cônjuge para visão completa das finanças familiares.

8. **Gerencie suas contas:** Use a plataforma web para ter controle total de todas as suas contas bancárias.

---

**🚀 Comece agora mesmo! Envie sua primeira mensagem e organize suas finanças de forma simples e inteligente.**
