      ...
      <div className="lg:col-span-2 bg-card border border-border rounded-xl p-4 md:p-6">
        ...
        <div className="flex items-center justify-center gap-4 md:gap-6 mt-4 md:mt-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs md:text-sm text-muted-foreground">{t('dashboard.stats.income')}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#EF4444]" />
            <span className="text-xs md:text-sm text-muted-foreground">{t('dashboard.stats.expenses')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4 md:p-6 flex flex-col">
        ...
        <div className="flex-1 relative" style={{ minHeight: '200px', height: '200px' }}>
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-popover border border-border p-2 rounded-lg shadow-xl">
                        <p className="text-popover-foreground font-medium text-sm">{data.name}</p>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-[10px] md:text-xs text-muted-foreground">{t('dashboard.charts.total')}</span>
          </div>
        </div>
          ...
          {expensesByCategory.map((cat, index) => (
            <div key={index} className="flex items-center justify-between">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                />
                <span className="text-xs md:text-sm text-muted-foreground truncate" title={cat.name}>
                  {cat.name}
                </span>
              </div>
              <div className="text-right flex-shrink-0 ml-2">
                ...
                <p className="text-[10px] md:text-xs text-muted-foreground">{cat.percentage}%</p>
              </div>
            </div>
          ))}

          {expensesByCategory.length === 0 && (
            <div className="text-center text-muted-foreground py-4 text-xs md:text-sm">
