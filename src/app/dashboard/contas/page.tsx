    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground">{t('accounts.description')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
        <div className="bg-card border border-border rounded-xl p-4 md:p-5 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4 relative">
            ...
            <span className="text-[10px] md:text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('accounts.totalBalance')}</span>
          </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4 md:p-5 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4 relative">
            ...
            <span className="text-[10px] md:text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('accounts.activeAccounts')}</span>
          </div>
          <div className="relative">
            <span className="text-2xl md:text-3xl font-bold text-foreground block">
              {accounts.length}
      ...
      {loading ? (
        ...
        searchTerm ? (
          <div className="text-center py-16 bg-card border border-border rounded-xl border-dashed">
            ...
            <p className="text-lg font-medium text-muted-foreground">{t('accounts.noAccounts')}</p>
          </div>
        ) : (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAccounts.map((account) => (
              <div key={account.id} className="bg-card border border-border rounded-2xl p-6 flex flex-col justify-between hover:border-primary/30 transition-all shadow-lg shadow-black/5 group relative overflow-hidden">

                {/* Badge Tipo Conta */}
                <div className={cn(
                  "absolute top-0 right-0 px-3 py-1 rounded-bl-xl text-[10px] font-bold uppercase tracking-wider",
                  account.tipo_conta === 'pj' ? "bg-purple-500/20 text-purple-600 dark:text-purple-400" : "bg-blue-500/20 text-blue-600 dark:text-blue-400"
                )}>
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <button onClick={fetchAccounts} title={t('common.loading')} className="hover:text-primary transition-colors">
                      <RefreshCw className="w-3 h-3 text-muted-foreground hover:rotate-180 transition-all duration-500" />
                    </button>
                  </div>
                ...
                <div className="space-y-3 pt-4 border-t border-border">
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    >
                      <FileText className="w-5 h-5 text-muted-foreground group-hover/btn:text-foreground transition-colors" />
